  ...
  return {
    ...
    coreFeatures: [
      {
        ...
        key: "path_traversal",
        description:
          "Blocks directory escape attacks (../) in all file operations. Two-phase validation: syntactic rejection of path components, then canonicalization to normalize symlinks.",
      ...
      },
      {
        ...
        key: "ssrf_protection",
        description:
          "Blocks outbound requests to private IPs, localhost, and cloud metadata endpoints (AWS/GCP/Azure). Validates DNS resolution results to defeat rebinding attacks.",
      ...
      },
      {
        ...
        description:
          "Deny-by-default permission system. Every agent operation (file I/O, network, shell, memory, spawn) requires an explicit capability grant in the manifest.",
      ...
      },
      {
        ...
        description:
          "When a parent agent spawns a child, the kernel enforces child capabilities are a subset of parent capabilities. No agent can grant rights it does not have.",
      ...
      },
      {
        ...
        description:
          "Child processes (shell tools) inherit only a safe allow-list of environment variables. API keys, database passwords, and secrets are never leaked to subprocesses.",
      ...
      },
      {
        ...
        description:
          "Every HTTP response includes CSP, X-Frame-Options: DENY, X-Content-Type-Options: nosniff, Referrer-Policy, and X-XSS-Protection headers.",
        threat: "XSS, clickjacking, MIME sniffing, content injection",
        impl: "middleware.rs — security_headers()",
      },
      {
        name: "Wire Protocol Authentication",
        key: "wire_hmac_auth",
        description:
          "Agent-to-agent OFP connections use HMAC-SHA256 mutual authentication with nonce-based handshake and constant-time signature comparison (subtle crate).",
      ...
      },
      {
        ...
        description:
          "Every API request receives a unique UUID (x-request-id header) and is logged with method, path, status code, and latency for full traceability.",
    ...
    configurableFeatures: [
      {
        ...
        key: "rate_limiter",
        description:
          "GCRA (Generic Cell Rate Algorithm) with cost-aware tokens. Different endpoints cost different amounts — spawning an agent costs 50 tokens, health check costs 1.",
        configHint:
          "Hard-coded: 500 tokens/minute per IP. Edit rate_limiter.rs to tune.",
      ...
      },
      {
        ...
        description:
          "Per-IP connection cap prevents connection exhaustion. Idle timeout closes abandoned connections. Message rate limiting prevents flooding.",
        configHint:
          "Hard-coded: 5 connections/IP, 30min idle timeout, 64KB max message. Edit ws.rs to tune.",
        valueKey: "websocket_limits",
      },
      {
        name: "WASM Dual Metering",
        key: "wasm_sandbox",
        description:
          "WASM modules run with two independent resource limits: fuel metering (CPU instruction count) and epoch interruption (wall-clock timeout with watchdog thread).",
        configHint:
          "Default: 1M fuel units, 30s timeout. Configurable per-agent via SandboxConfig.",
      ...
      },
      {
        ...
        key: "auth",
        description:
          "All non-health endpoints require Authorization: Bearer header. When no API key is configured, all requests are restricted to localhost only.",
        configHint:
          "Set api_key in ~/.openfang/config.toml for remote access. Empty = localhost only.",
        valueKey: "auth",
      },
    ],
    ...
    monitoringFeatures: [
      {
        ...
        key: "audit_trail",
        description:
          "Every security-critical action is appended to an immutable, tamper-evident log. Each entry is cryptographically linked to the previous via SHA-256 hash chain.",
        configHint:
          "Always active. Verify chain integrity from the Audit Log page.",
        valueKey: "audit_trail",
      },
      {
        name: "Information Flow Taint Tracking",
        key: "taint_tracking",
        description:
          "Labels data by provenance (ExternalNetwork, UserInput, PII, Secret, UntrustedAgent) and blocks unsafe flows: external data cannot reach shell_exec, secrets cannot reach network.",
        configHint: "Always active. Prevents data flow attacks automatically.",
        valueKey: "taint_tracking",
      },
      {
        name: "Ed25519 Manifest Signing",
        key: "manifest_signing",
        description:
          "Agent manifests can be cryptographically signed with Ed25519. Verify manifest integrity before loading to prevent supply chain tampering.",
        configHint:
          "Available for use. Sign manifests with ed25519-dalek for verification.",
    ...
    async loadSysInfo() {
      try {
        ...
        this.sysInfo = {
          ...
          railway_deploy_hook_configured:
            !!status.railway_deploy_hook_configured,
        };
      } catch (e) {
        throw e;
      }
    ...
    async loadConfigSchema() {
      try {
        var results = await Promise.all([
          OpenFangAPI.get("/api/config/schema").catch(function () {
            return {};
          }),
    ...
    get filteredModels() {
      ...
      return this.models.filter(function (m) {
        if (self.modelProviderFilter && m.provider !== self.modelProviderFilter)
          return false;
        if (self.modelTierFilter && m.tier !== self.modelTierFilter)
          return false;
        if (self.modelSearch) {
    ...
    async saveProviderKey(provider) {
      ...
      try {
        await OpenFangAPI.post(
          "/api/providers/" + encodeURIComponent(provider.id) + "/key",
          { key: key.trim() },
        );
    ...
    async startCopilotOAuth() {
      ...
      try {
        var resp = await OpenFangAPI.post(
      ...
      setTimeout(async function () {
        ...
        try {
          var resp = await OpenFangAPI.get(
            "/api/providers/github-copilot/oauth/poll/" +
              self.copilotOAuth.pollId,
          );
    ...
    async testProvider(provider) {
      ...
      try {
        ...
        this.providerTestResults[provider.id] = {
          status: "error",
          error: e.message,
        };
    ...
    async saveProviderUrl(provider) {
      ...
      if (!url || !url.trim()) {
        OpenFangToast.error("Please enter a base URL");
        return;
      }
      url = url.trim();
      if (url.indexOf("http://") !== 0 && url.indexOf("https://") !== 0) {
        OpenFangToast.error("URL must start with http:// or https://");
        return;
      }
      this.providerUrlSaving[provider.id] = true;
      try {
        var result = await OpenFangAPI.put(
          "/api/providers/" + encodeURIComponent(provider.id) + "/url",
          { base_url: url },
        );
        if (result.reachable) {
          OpenFangToast.success(
            provider.display_name +
              " URL saved &mdash; reachable (" +
              (result.latency_ms || "?") +
              "ms)",
          );
        } else {
          OpenFangToast.warning(
            provider.display_name + " URL saved but not reachable",
          );
        }
        await this.loadProviders();
      } catch (e) {
      ...
      switch (feature.valueKey) {
        ...
        case "websocket_limits":
          return (
            ...
            (val.max_per_ip || 5) +
            " conn/IP | " +
            Math.round((val.idle_timeout_secs || 1800) / 60) +
            "min idle timeout | " +
            Math.round((val.max_message_size || 65536) / 1024) +
            "KB max msg"
          );
        case "wasm_sandbox":
          return (
            "Fuel: " +
            (val.fuel_metering ? "ON" : "OFF") +
            " | Epoch: " +
            (val.epoch_interruption ? "ON" : "OFF") +
            " | Timeout: " +
            (val.default_timeout_secs || 30) +
            "s"
          );
        case "auth":
          return (
            "Mode: " +
            (val.mode || "unknown") +
            (val.api_key_set ? " (key configured)" : " (no key set)")
          );
        default:
    ...
    async scanPath() {
      ...
      this.scanning = true;
      try {
        var data = await OpenFangAPI.post("/api/migrate/scan", {
    ...
    async runMigration(dryRun) {
      ...
      try {
        ...
        var data = await OpenFangAPI.post("/api/migrate", {
          source: "openclaw",
          source_dir:
            this.sourcePath || (this.scanResult ? this.scanResult.path : ""),
          target_dir: target,
          dry_run: dryRun,
        });
        this.migResult = data;
